...
async def ADregister(user_data: UserRegistration, conn: pymysql.connections.Connection):
    try:
        account = user_data.account
        password = user_data.password
        print(account)
        ...
        with conn.cursor() as cursor:
            # 查询账号是否已存在
            sql = "SELECT COUNT(*) FROM admin WHERE account = %s"
            cursor.execute(sql, (account,))
            result = cursor.fetchone()
            if result[0] > 0:
                return {'message': '账号已存在'}
            ...
            cursor.execute(sql, (account, hashed_password))
            # 提交事务
            conn.commit()
        return {'message': '注册成功'}
    except Exception as e:
